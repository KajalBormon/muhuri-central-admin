        ...
        <div class="card mb-5 mb-xl-10">
            ...
            <div class="card-header border-0 cursor-pointer">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">{{ props.tenantPermission?.id ? 'Edit Tenant Permission' : 'Add Tenant Permission' }}</h3>
                </div>
            </div>
            <!--end::Card header-->
            ...
            <div class="show">
                <VForm @submit="submit()" class="form">
                    ...
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <SubmitButton :id="props.tenantPermission?.id"/>
                    </div>
                </VForm>
            </div>
        </div>
...
const formData = useForm({
    name: props.tenantPermission?.name || '',
    group_name: props.tenantPermission?.group_name || '',
});
...
const submit = () => {
    if (props.tenantPermission?.id) {
        // for updating tenant permission
        formData.put(route('tenant-permissions.update', props.tenantPermission?.id), {
            preserveScroll: true,
        });
    } else {
        // for adding new tenant permission
        formData.post(route('tenant-permissions.store'), {
            preserveScroll: true,
        });
    }
};
